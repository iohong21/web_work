...
<html>
  ...
  <body>
		...
		<form action="update.jsp" method="post" id="updateForm">
			<input type="hidden" name="id" id="id" value="<%=dto.getId() %>" /><br />
			<label for="id">아이디: </label>
			<input type="text" value="<%=dto.getId() %>" disabled="disabled"/>
			<label for="pwd">비밀번호: </label>
			<input type="password" name="pwd" id="pwd" value="<%=dto.getPwd() %>" /><br />
			<label for="pwd2">비밀번호 확인: </label>
			<input type="password" name="pwd2" id="pwd2" value="<%=dto.getPwd() %>" /><br />
